...
- Base CRUD (create/rename/upload), document enable/disable, versioning, and audit logging for data products (PRD §4.6, REQ-DP-1/DP-2/DP-5) are specified in full by spec 019 (Data Products — Core Capability); this spec supplies only the authorization/security layer those actions must pass through, not the base capability itself.
