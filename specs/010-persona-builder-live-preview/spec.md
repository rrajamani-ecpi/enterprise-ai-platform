...
- **PRD §4.7 cross-check — REQ-PERSONA-6**: AI-assisted persona generation from a natural-language description using an admin-configured generation model is fully covered by this spec's User Stories 1–2 and FR-001 through FR-004 (structured-output generation with no-fabrication guarantee, allow-listed/Opus-excluded model catalog); no additional spec work is needed for this requirement.
