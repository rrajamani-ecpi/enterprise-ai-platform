...
- The base capability of bulk ingestion as a user/owner-facing feature (that documents "may also arrive via the enterprise content ingestion pipeline, not only manual upload," and that per-collection ingestion health is surfaced to owners) is specified by spec 019 (Data Products — Core Capability, REQ-DP-3/REQ-DP-8); this spec owns only the reliability/recovery mechanics (retry, dead-letter, idempotency) underneath that already-existing pipeline.
